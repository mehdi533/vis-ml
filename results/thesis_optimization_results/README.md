...
Canonical source configs now live under `configs/scheduling/` at repo root.
The local `results/thesis_optimization_results/configs/` path is kept for backward compatibility via symlinks for source configs, while `results/thesis_optimization_results/configs/generated/` remains artifact-generated.
...
- `configs/scheduling/suites/01_formulation_comparison.yaml`
...
- `scripts/run_working_formulations.sh`
  Runs only the smoke-validated working thesis suites end-to-end and skips the currently infeasible She/area Method IV comparisons.

- `scripts/run_working_formulations_local.sh`
  Local wrapper for the same working-suite run, still using the full production configs and outputs.

- `scripts/run_working_formulations_mtlsh.sh`
  Dedicated wrapper for the MTLSH thesis runs under `results/by_model/mtlsh`.

- `scripts/run_working_formulations_picnn.sh`
  Dedicated wrapper for the PICNN-family thesis runs under `results/by_model/picnn`. It currently exits during preflight because PICNN embeddings are not yet implemented in the scheduler MILP builder.

- `scripts/run_postprocess.sh`
  Builds thesis tables and figures from available optimization and replay outputs.

- `scripts/run_model_comparison_postprocess.sh`
  Builds a model-comparison pack for selected surrogate families, currently intended for MTLSH and PICNN runs under `results/by_model/`.
...
3. Working formulations only, full production outputs:
```bash
results/thesis_optimization_results/scripts/run_working_formulations.sh
```

4. Local working formulations, full production outputs:
```bash
results/thesis_optimization_results/scripts/run_working_formulations_local.sh
```

4b. MTLSH-only working formulations:
```bash
results/thesis_optimization_results/scripts/run_working_formulations_mtlsh.sh
```

4c. PICNN-only working formulations:
```bash
results/thesis_optimization_results/scripts/run_working_formulations_picnn.sh
```

5. Local debug smoke test:
```bash
results/thesis_optimization_results/scripts/run_debug.sh
```

5b. Fast local debug smoke test:
```bash
...
12. Build thesis tables and figures from the generated outputs:
```bash
results/thesis_optimization_results/scripts/run_postprocess.sh
```

12b. Build MTLSH vs PICNN comparison tables and figures from `results/by_model/`:
```bash
results/thesis_optimization_results/scripts/run_model_comparison_postprocess.sh
```
...
The working-formulations wrappers intentionally include:
- formulation comparison
- security checks
- redispatch sensitivity
- zone-mismatch VIS sensitivity
- replay validation
- thesis postprocessing

They intentionally skip:
- `she_vis_rted_style_comparison`
- `area_vis_comparison`

Those two suites are excluded because the current exact-MILP Method IV variants are still infeasible in local smoke validation.
...
- `results/thesis_optimization_results/outputs/model_comparison/tables/model_formulation_tradeoff.*`
- `results/thesis_optimization_results/outputs/model_comparison/tables/model_security_retained.*`
- `results/thesis_optimization_results/outputs/model_comparison/tables/model_redispatch_effect.*`
- `results/thesis_optimization_results/outputs/model_comparison/tables/model_zone_vis_totals.*`
- `results/thesis_optimization_results/outputs/model_comparison/figures/model_formulation_tradeoff.*`
- `results/thesis_optimization_results/outputs/model_comparison/figures/model_security_profile.*`
- `results/thesis_optimization_results/outputs/model_comparison/figures/model_redispatch_effect.*`
- `results/thesis_optimization_results/outputs/model_comparison/figures/model_zone_vis_totals.*`
...
Configuration:
- `configs/scheduling/suites/02_she_vis_rted_style_comparison.yaml`
...
Debug suite config:
- `configs/scheduling/suites/legacy/formulation_comparison_debug.yaml`

Fast local suite config:
- `configs/scheduling/suites/legacy/formulation_comparison_debug_local.yaml`
...
```bash
SUITE_CONFIG=configs/scheduling/suites/01_formulation_comparison.yaml \
results/thesis_optimization_results/scripts/run_debug.sh
```
...
- missing retained surrogate artifacts
  check `model.model_dir` in `configs/scheduling/base_optimization.yaml`
- missing ANDES case
  check `system.case` in `configs/scheduling/base_optimization.yaml`
...
`configs/scheduling/base_optimization.yaml` now defaults to `allow_missing_features: false` so a bad feature contract fails explicitly instead of silently zero-filling unsupported inputs.
