...
def _plot_unit_delta_heatmaps(unit_df: pd.DataFrame, *, global_scenario_id: str, stem: str) -> list[str]:
    ...
    for row_idx, formulation_id in enumerate(formulations):
        ...
        for col_idx, metric in enumerate(["M_opt_vs_global", "D_opt_vs_global"]):
            ...
            title_metric = "Inertia shift vs global" if metric == "M_opt_vs_global" else "Damping shift vs global"
            ax.set_title(f"{formulation_id}: {title_metric}")
...
def _plot_area_totals(area_df: pd.DataFrame, *, global_scenario_id: str, stem: str) -> list[str]:
    ...
    for ax, value_col, title in [
        (axes[0], "M_sum", "Area-wise scheduled inertia"),
        (axes[1], "D_sum", "Area-wise scheduled damping"),
    ]:
        ...
        ax.set_title(title)
    axes[0].set_ylabel("Scheduled value sum")
