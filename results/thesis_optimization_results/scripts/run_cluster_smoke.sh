...
SUITE_CONFIG="results/thesis_optimization_results/configs/smoke/formulation_comparison_smoke.yaml" \
STOP_ON_ERROR="${STOP_ON_ERROR}" LOG_TAIL_LINES="${LOG_TAIL_LINES}" \
  bash "${SCRIPT_DIR}/run_formulations.sh"

SUITE_CONFIG="results/thesis_optimization_results/configs/smoke/security_checks_smoke.yaml" \
STOP_ON_ERROR="${STOP_ON_ERROR}" LOG_TAIL_LINES="${LOG_TAIL_LINES}" \
  bash "${SCRIPT_DIR}/run_security_checks.sh"

SUITE_CONFIG="results/thesis_optimization_results/configs/smoke/redispatch_sensitivity_smoke.yaml" \
STOP_ON_ERROR="${STOP_ON_ERROR}" LOG_TAIL_LINES="${LOG_TAIL_LINES}" \
  bash "${SCRIPT_DIR}/run_redispatch_sensitivity.sh"

SUITE_CONFIG="results/thesis_optimization_results/configs/smoke/she_vis_rted_style_comparison_smoke.yaml" \
STOP_ON_ERROR="${STOP_ON_ERROR}" LOG_TAIL_LINES="${LOG_TAIL_LINES}" \
  bash "${SCRIPT_DIR}/run_she_style_comparison.sh"

SUITE_CONFIG="results/thesis_optimization_results/configs/smoke/area_vis_comparison_smoke.yaml" \
...
SUITE_CONFIG="results/thesis_optimization_results/configs/smoke/zone_mismatch_vis_sensitivity_smoke.yaml" \
...
REPLAY_CONFIG="results/thesis_optimization_results/configs/smoke/replay_validation_smoke.yaml" \
