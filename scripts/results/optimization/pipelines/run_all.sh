...
FORMULATION_SUITE_CONFIG="${FORMULATION_SUITE_CONFIG:-configs/scheduling/suites/01_formulation_comparison.yaml}"
SHE_SUITE_CONFIG="${SHE_SUITE_CONFIG:-configs/scheduling/suites/02_she_vis_rted_style_comparison.yaml}"
AREA_VIS_SUITE_CONFIG="${AREA_VIS_SUITE_CONFIG:-configs/scheduling/suites/03_area_vis_comparison.yaml}"
...
ZONE_MISMATCH_SUITE_CONFIG="${ZONE_MISMATCH_SUITE_CONFIG:-configs/scheduling/suites/04_zone_mismatch_vis_sensitivity.yaml}"
...
SECURITY_SUITE_CONFIG="${SECURITY_SUITE_CONFIG:-configs/scheduling/suites/05_security_checks.yaml}"
REDISPATCH_SUITE_CONFIG="${REDISPATCH_SUITE_CONFIG:-configs/scheduling/suites/06_redispatch_sensitivity.yaml}"
REPLAY_CONFIG="${REPLAY_CONFIG:-configs/scheduling/replay/replay_validation.yaml}"
ANALYSIS_CONFIG="${ANALYSIS_CONFIG:-configs/scheduling/analysis/results_pack.yaml}"
...
SUITE_CONFIG="${AREA_VIS_SUITE_CONFIG}" SUMMARY_JSON="${AREA_VIS_SUMMARY_JSON}" BASELINE_ID="${AREA_VIS_BASELINE_ID}" STEM="${AREA_VIS_STEM}" STOP_ON_ERROR="${STOP_ON_ERROR}" LOG_TAIL_LINES="${LOG_TAIL_LINES}" \
  bash "${SCRIPT_DIR}/03_run_area_vis_comparison.sh"
...
SUITE_CONFIG="${ZONE_MISMATCH_SUITE_CONFIG}" SUMMARY_JSON="${ZONE_MISMATCH_SUMMARY_JSON}" GLOBAL_SCENARIO_ID="${ZONE_MISMATCH_GLOBAL_SCENARIO_ID}" STEM="${ZONE_MISMATCH_STEM}" STOP_ON_ERROR="${STOP_ON_ERROR}" LOG_TAIL_LINES="${LOG_TAIL_LINES}" \
  bash "${SCRIPT_DIR}/04_run_zone_mismatch_vis_sensitivity.sh"

SUITE_CONFIG="${SECURITY_SUITE_CONFIG}" STOP_ON_ERROR="${STOP_ON_ERROR}" LOG_TAIL_LINES="${LOG_TAIL_LINES}" \
  bash "${SCRIPT_DIR}/05_run_security_checks.sh"

SUITE_CONFIG="${REDISPATCH_SUITE_CONFIG}" STOP_ON_ERROR="${STOP_ON_ERROR}" LOG_TAIL_LINES="${LOG_TAIL_LINES}" \
  bash "${SCRIPT_DIR}/06_run_redispatch_sensitivity.sh"

REPLAY_CONFIG="${REPLAY_CONFIG}" bash "${SCRIPT_DIR}/07_run_replay_validation.sh"

ANALYSIS_CONFIG="${ANALYSIS_CONFIG}" REQUIRE_REPLAY="${REQUIRE_REPLAY}" bash "${SCRIPT_DIR}/08_run_postprocess.sh"
