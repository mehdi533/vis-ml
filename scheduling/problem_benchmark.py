...
from models.models import MTLSharedHeads, MLP
...
def surrogate_constraints_milp2(model_freq, model_dp,
                               x,
                               x_min_sc,
                               x_max_sc,
                               y, # TODO: add type
    ...
    for model, y_model in ((model_freq, y[:4]), (model_dp, y[4:])):
        ...
        for idx, (W, b) in enumerate(layers):
            ...
            if idx == len(layers) - 1: # last layer of the head, we want to constrain it to be equal to the output variable y
                constraints.append(y_model == z)
                continue 
...
def define_rted_vis(ss: andes.System, model_freq, model_dp,
                    x_val_sc: np.ndarray,
    ...
    constraints_nn = surrogate_constraints_milp2(model_freq, model_dp, x, x_min_sc, x_max_sc, y)
    ...
    checks = {
        "input": constraints_x + constraints_ed,
        "output": constraints_y + constraints_ed,
        # NN alone is usually feasible because x/y are unconstrained; keep it as a baseline.
        "nn_free": constraints_nn + constraints_ed,
        # More meaningful checks (these are the ones that typically reveal the conflict):
        "nn_given_input": constraints_x + constraints_nn + constraints_ed,
        "output_nn_given_input": constraints_x + constraints_y + constraints_nn + constraints_ed,
        # Full problem:
        "line_flows": constraints_line + constraints_ed,
        "combined": constraints,
    }

    solver_kwargs = {"solver": "GUROBI", "verbose": False, "reoptimize": True}
...
def main():
    ...
    model_cfg = cost_cfg.get("models", {})
    # Config convention (see scheduling/mtlsh_convex.yaml):
    # - state_dict1: deltaP model (4 outputs)
    # - state_dict2: freq model (4 outputs)
    state_path_dp = Path(model_cfg.get("state_dict1", ""))
    state_path_freq = Path(model_cfg.get("state_dict2", ""))
    
    if not (state_path_dp.exists() and state_path_freq.exists()):
        raise FileNotFoundError(
            "Model state_dict(s) not found. Expected:\n"
            f"- {state_path_dp}\n"
            f"- {state_path_freq}"
        )
    ...
    model_dp.load_state_dict(torch.load(state_path_dp, map_location="cpu"))
    model_dp.eval()
    ...
    model_freq.load_state_dict(torch.load(state_path_freq, map_location="cpu"))
    model_freq.eval()
    ...
    y_scaler_full = joblib.load(scalers_cfg.get("y_scaler_path"))
    y_scaler_freq_path = scalers_cfg.get("y_scaler_freq_path")
    y_scaler_dp_path = scalers_cfg.get("y_scaler_dp_path")
    y_scaler_freq = joblib.load(y_scaler_freq_path) if y_scaler_freq_path else None
    y_scaler_dp = joblib.load(y_scaler_dp_path) if y_scaler_dp_path else None
    ...
    m_vec = np.full(ss.REGCV1.n, 2.0)
    # m_vec = [2,2,2,2]
    d_vec = np.full(ss.REGCV1.n, 0.0)
    ...
    if (
        y_scaler_freq is not None
        and y_scaler_dp is not None
        and y_min_raw.size == 8
        and y_max_raw.size == 8
    ):
        y_min_sc = np.concatenate(
            [
                scale_minmax(y_min_raw[:4], y_scaler_freq, np.arange(4)),
                scale_minmax(y_min_raw[4:], y_scaler_dp, np.arange(4)),
            ]
        )
        y_max_sc = np.concatenate(
            [
                scale_minmax(y_max_raw[:4], y_scaler_freq, np.arange(4)),
                scale_minmax(y_max_raw[4:], y_scaler_dp, np.arange(4)),
            ]
        )
    else:
        y_min_sc = scale_minmax(y_min_raw, y_scaler_full, np.arange(len(y_min_raw)))
        y_max_sc = scale_minmax(y_max_raw, y_scaler_full, np.arange(len(y_min_raw)))
    ...
    if y_scaler_dp is not None:
        dp_scale = y_scaler_dp.scale_
        dp_min_sc = y_scaler_dp.min_
    else:
        dp_scale = y_scaler_full.scale_[4:8]
        dp_min_sc = y_scaler_full.min_[4:8]
    p_ibr_sc_min = cp.multiply(p_ibr_min, dp_scale) + dp_min_sc
    p_ibr_sc_max = cp.multiply(p_ibr_max, dp_scale) + dp_min_sc
    dp_ibr_sc = cp.multiply(dp_ibr, dp_scale) + dp_min_sc
    ...
    prob, values = define_rted_vis(
        ss, model_freq, model_dp, x_val_sc,
    ...
    print(values["feasibility"])

    x_unscaled = (values["x"].value - x_scaler.min_) / x_scaler.scale_
    if y_scaler_freq is not None and y_scaler_dp is not None:
        y_val_sc = np.asarray(values["y"].value, dtype=float).reshape(-1)
        y_unscaled = np.empty_like(y_val_sc)
        y_unscaled[:4] = unscale_minmax(y_val_sc[:4], y_scaler_freq, np.arange(4))
        y_unscaled[4:] = unscale_minmax(y_val_sc[4:], y_scaler_dp, np.arange(4))
    else:
        y_unscaled = (values["y"].value - y_scaler_full.min_) / y_scaler_full.scale_
